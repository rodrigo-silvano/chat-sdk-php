...
class PHPMailer
{
    ...
    public function send()
    {
        try {
            ...
            $this->mailHeader = '';
            $this->setError($this->sanitizedExceptionLabel($exc));
            if ($this->exceptions) {
                throw $exc;
            }
    ...
    public function postSend()
    {
        try {
            ...
            $label = $this->sanitizedExceptionLabel($exc);
            $this->setError($label);
            $this->edebug($label);
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    ...
                    $lastexception = $exc;
                    $this->edebug($this->sanitizedExceptionLabel($exc));
    ...
    protected function sanitizedExceptionLabel(Exception $exc)
    {
        $label = $exc::class;
        if ($exc->getCode() !== 0) {
            $label .= ' #' . (string) $exc->getCode();
        }
